...
import kz.aphion.adverts.models.subscription.FlatRentSubscriptionModel;
...
public class FlatRentRealtyEmailNotificationChannelBuilder {
	...
	public NotificationChannel build(Subscription subscription, List<SubscriptionAdvert> subscriptionAdverts) throws TemplateException, IOException {
		...
		FlatRentSubscriptionModel subscriptionModel = new FlatRentSubscriptionModel(subscription);
	...
	private String buildTitle(SubscriptionModel subscriptionModel, List<SubscriptionAdvert> subscriptionAdverts) {		
		...
		// TEMP		
		title.append(" [" + subscriptionModel.getSubscription().id + "]");
		
		return title.toString();
	}
	
	private String buildContent(FlatRentSubscriptionModel subscriptionModel, List<SubscriptionAdvert> subscriptionAdverts) throws TemplateException, IOException {

		Map<String, Object> root = new HashMap<>();

		// Put global object for access from the template
		root.put("subscription", subscriptionModel.getSubscription());
		root.put("user", subscriptionModel.getSubscription().user);
		root.put("notification", subscriptionModel.getSubscription().notification);
		
		root.put("subscriptionAdverts", subscriptionAdverts);
		root.put("adverts", buildAdvertModels(subscriptionModel.getSubscription(), subscriptionAdverts));
		
		// Add specific objects for predefined fields
		root.put("subscriptionTitle", ""); // ???
		root.put("subscriptionDescriptionText", getSubscriptionDescriptionText(subscriptionModel));
	...
	private FlatSellRealtyModel buildAdvertModel(Subscription subscription, SubscriptionAdvert subscriptionAdvert) {
		...
		FlatRentAdvertModel realtyModel = new FlatRentAdvertModel(realty);
	...
	private String getAdvertTitle(FlatRentAdvertModel model) {
	...
	private String getAdvertShortDescription(FlatRentAdvertModel model) {
	 ...
	 * @return
	 */
	private String getSubscriptionDescriptionText(FlatRentSubscriptionModel subscriptionModel) {
		...
		// Rent Periods
		List<FlatRentPeriodType> flatRentPeriodTypes = subscriptionModel.getCriteriaModel().getRentPeriods();
		if (flatRentPeriodTypes != null) {
			descr.append(getDescriptionRentPeriods(flatRentPeriodTypes));
		}
		descr.append(", ");
	...
	private Object getDescriptionRentPeriods(List<FlatRentPeriodType> flatRentPeriodTypes) {
		String rp = flatRentPeriodTypes.size() > 0 ?  " (" : "";
		
		for (FlatRentPeriodType flatRentPeriodType : flatRentPeriodTypes) {
	...
	private List<NotificationParameter> buildParameters(SubscriptionModel subscriptionModel, List<SubscriptionAdvert> subscriptionAdverts) {
