...
public class FlatRentRealtyEmailNotificationChannelBuilder {
	...
	public NotificationChannel build(Subscription subscription, List<SubscriptionAdvert> subscriptionAdverts) throws TemplateException, IOException {
		if (subscription.criteria instanceof RealtyRentFlatSubscriptionCriteria == false) {
			logger.error("FlatRentRealtyEmailNotificationChannelBuilder was selected by mistake, please check subscription.id: " + subscription.id);
			return null;
		}
	...
	private String getSubscriptionDescriptionText(Subscription subscription) {
		...
		if (subscription.criteria instanceof RealtySellFlatSubscriptionCriteria) {
			...
			// Rent Periods
			if (((RealtyRentFlatSubscriptionCriteria)baseCriteria).rentPeriods!=null) {
				descr.append(getDescriptionRentPeriods(((RealtyRentFlatSubscriptionCriteria)baseCriteria).rentPeriods));
			}
			descr.append(", ");
			// Location			
	...
	private Object getDescriptionRentPeriods(List<FlatRentPeriodType> rentPeriods) {
		String rp = rentPeriods.size() > 0 ?  " (" : "";
		for (FlatRentPeriodType flatRentPeriodType : rentPeriods) {
