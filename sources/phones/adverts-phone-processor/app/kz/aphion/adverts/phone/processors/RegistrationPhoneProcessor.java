...
public class RegistrationPhoneProcessor {
	...
	public void processRegistrationMessage(String registrationMessage) throws Exception {
		...
		for (String phoneNumber : model.phone) {
			...
			if (phoneEntity == null) {
				...
				Logger.info("Phone number [%s] successfully added to db", phone);
			} else {
				// Update existing
				phoneEntity = updateExistingPhoneRecord(phoneEntity, model);
				Logger.info("Record with phone number [%s] successfully updated in db", phone);
			}
	...
	private Phone updateExistingPhoneRecord(Phone existingPhoneEntity, RegisterPhoneModel model) throws Exception{
		...
		if (wasUpdated == false) {
			Logger.info("Record with phone number [%s] was not updated. Skip update process.", existingPhoneEntity.number);
...
//				.set("updated", existingPhoneEntity.updated);
	...
	private String normalizePhoneNumber(String phoneNumber) {
		Logger.info("Phone number before normalization: %s", phoneNumber);
		String phone = phoneNumber.replaceAll("[^\\d]", "");
		
		if (phone.length() > 10)
			phone = phone.substring(phone.length()-10, phone.length());
		
		Logger.info("Phone number after normalization: %s", phone);
		return phone;
	}
