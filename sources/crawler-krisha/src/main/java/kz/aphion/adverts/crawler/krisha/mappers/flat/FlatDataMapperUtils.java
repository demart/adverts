...
public class FlatDataMapperUtils {
	...
	public static void mapResidentialComplex(Map<String, Object> advert, HashMap<String, Object> data, String externalComplexId) {
		...
		KrishaResidentialComplex complexEntity = KrishaDataManager.getResidentalComplex(externalComplexId);
		...
		if (complexEntity == null) {
			...
			if (krishaRegion != null) {
				...
				if (complexEntity == null) {
					...
					// TODO Зделать по нормальному!
					//complex.relationId = complexEntity.id;
					//complex.name = complexEntity.name;
					logger.error("KrishaResidentalComplex with name [{}] and region.id [{}] not found and not saved in advert!", externalComplexId, krishaRegion.region.id);
				}
			} else {
				logger.error("KrishaResidentialComplex with id [{}] not found!", externalComplexId);
			}
		} else {
			if (complexEntity.complex != null) {
				...
				logger.error("KrishaResidentialComplex with id [{}] doens't have link to ResidentialComplex.");
			}
		}
	}
